...
class VQVAEModule(nn.Module):
    def __init__(self, n_mels, channel_size, resblock_kernel_sizes, embed_dim, n_embed):
        ...
        self.ups = nn.ModuleList([weight_norm(ConvTranspose1d(embed_dim, 2 * embed_dim, 4, 2, padding=1)),
                                  weight_norm(ConvTranspose1d(2 * embed_dim, 2 * embed_dim, 4, 2, padding=1)),
                                  weight_norm(ConvTranspose1d(2 * embed_dim, embed_dim, 4, 2, padding=1))])
        for i in range(3):
            resblock_list = nn.ModuleList()
            for kernel_size in resblock_kernel_sizes:
                if i < 2:
                    resblock_list.append(ResBlock(2 * embed_dim, kernel_size))
                else:
    ...
    def forward(self, x):
        ...
        for resblocks, up in zip(self.resblocks_decoder, self.ups):
            x = up(x)
